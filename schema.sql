...
CREATE TABLE PARAMETRES(
    ...
    THEME VARCHAR(100),
    REPAS VARCHAR(10),
    MODE VARCHAR(20) NOT NULL DEFAULT 'edition',
    ...
    CONSTRAINT CK_PARAMETRES_MODE CHECK (MODE IN ('edition', 'nouveau_message')),
    CONSTRAINT CK_PARAMETRES_NOTIFICATION CHECK (NOTIFICATION IN ('nouveau', 'maj', 'repas'))
);
...
-- Compteur cumulatif de sessions analytics par ville
CREATE TABLE GEO_USAGE(
    CITY VARCHAR(100) PRIMARY KEY,
    TOTAL BIGINT NOT NULL DEFAULT 0,
    CONSTRAINT FK_GEO_USAGE_GEO_DATA FOREIGN KEY (CITY) REFERENCES GEO_DATA(CITY)
);

-- Indexes
CREATE INDEX idx_menu_date ON MENU (DATE);
CREATE INDEX idx_menu_rid_date ON MENU (RID, DATE);
CREATE INDEX idx_repas_tpr ON REPAS (TPR);
CREATE INDEX idx_repas_mid ON REPAS (MID);
CREATE INDEX idx_categorie_rpid ON CATEGORIE (RPID);
